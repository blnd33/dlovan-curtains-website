...
const API_BASE = window.location.port === '4000' || window.location.hostname !== 'localhost'
  ? ''       // same origin when served by Node server
  : 'http://localhost:4000'; // dev fallback

/* ════════════════════════════════
   ADMIN CONTENT LOADER
   Fetches saved content from backend API and applies it
════════════════════════════════ */
async function fetchAndApplyContent() {
  try {
    const [contentRes, imagesRes] = await Promise.all([
      fetch(`${API_BASE}/api/content`),
      fetch(`${API_BASE}/api/upload`),
    ]);
    const flat   = contentRes.ok  ? await contentRes.json() : {};
    const images = imagesRes.ok   ? await imagesRes.json()  : {};

    // Rebuild nested content object from flat DB keys
    const c = {};
    ...
    for (const [k, url] of Object.entries(images)) {
      const [section, field] = k.split('.');
      if (!c[section]) c[section] = {};
      c[section][field] = `${API_BASE}${url}`;
    }

    applyAdminContent(c);
  } catch (e) {
    console.warn('Could not load content from server:', e.message);
  }
}

function applyAdminContent(c) {
  if (!c) return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── Apply language first, then fetch content from server ── */
  const saved = localStorage.getItem('dp-lang');
  try { applyLang(saved && saved !== 'en' ? saved : 'en'); } catch(e) { console.warn('Lang error:', e); }

  fetchAndApplyContent().then(() => {
    // Re-apply language after content is loaded so translations merge correctly
    try { applyLang(localStorage.getItem('dp-lang') || 'en'); } catch(e) {}
  });
